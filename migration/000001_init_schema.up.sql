...
CREATE TABLE "history" (
  ...
  "type" varchar NOT NULL,
  "phone" varchar NOT NULL,
  "passenger_id" bigint NOT NULL DEFAULT 0,
...
CREATE INDEX ON "history" ("passenger_id");

CREATE INDEX ON "history" ("pick_up_longitude", "pick_up_latitude");

CREATE INDEX ON "history" ("drop_off_longitude", "drop_off_latitude");

COMMENT ON COLUMN "request"."passenger_id" IS 'null if type is "phone" ';

COMMENT ON COLUMN "response"."driver_id" IS '0 if found_driver is false';

COMMENT ON COLUMN "history"."passenger_id" IS '0 if passenger is non-user';
